...
export function useErrorHandler() {
  ...
  const logError = useCallback((error: Error | ErrorInfo, context?: string) => {
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      // window.analytics?.track('Error', { error, context });
    }
  }, []);
  ...
  const handleError = useCallback((
    ...
    // Normalizar diferentes tipos de erro
    if (error instanceof Error) {
      ...
      errorInfo = {
        message: options?.fallbackMessage || (language === 'pt' ? 'Erro desconhecido' : 'Unknown error'),
        context,
        details: error,
        retryAction: options?.retryAction,
      };
  ...
  const handleNetworkError = useCallback((
    ...
    if (isNetworkError) {
      ...
      if (options?.retryAction) {
        setTimeout(options.retryAction, 3000); // Auto-retry após 3s
      }
    } else {
...
function getErrorTitle(error: ErrorInfo, language: string): string {
  const titles = {
    pt: {
...
function getErrorMessage(error: ErrorInfo, language: string): string {
  // Mensagens mais amigáveis baseadas no contexto
