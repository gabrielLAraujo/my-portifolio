  ...
  return (
    <>
      <motion.div
        ...
        role="toolbar"
        aria-label={language === 'pt' ? 'Controles de interface' : 'Interface controls'}
      >
        <motion.button
          onClick={toggleTheme}
          onKeyDown={(e) => {
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          aria-label={
            language === 'pt' ? 'Mudar para inglês (Alt + L)' : 'Mudar para português (Alt + L)'
          }
          aria-pressed={language === 'en'}
        >
          <FaGlobeAmericas className="text-xl text-green-500" aria-hidden="true" />
          <span className="sr-only">
            {language === 'pt' ? 'Idioma atual: Português' : 'Current language: English'}
          </span>
        </motion.button>
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.9 }}
          aria-label={language === 'pt' ? 'Mostrar atalhos de teclado' : 'Show keyboard shortcuts'}
          ...
          <ul className="space-y-2">
            {keyboardShortcuts.map((shortcut, index) => (
              <li key={index} className="flex justify-between items-center text-sm">
                ...
                <span className="text-gray-600 dark:text-gray-300 ml-2">{shortcut.action}</span>
              </li>
            ))}
          </ul>
          <button
